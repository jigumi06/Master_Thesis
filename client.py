...
def client_fedfa_cl(args, client_index, anchorloss_funcs, client_models, global_model, global_round, dataset_train, dict_users, loss_dict, mask):  # update nn
    for k in client_index: #k is the index of the client
        if args.verbose:
            print('client {} client_fedfa_anchorloss...'.format(k))
        ...
        model_count = 0
        mask_count = 0
        for layer in client_models[k].parameters():
            model_count += 1
            #print("model_layer: ", layer)
            for mask_layer in mask:
                mask_count += 1
                if mask_count == model_count:
                    #print("mask_layer: ", mask_layer)
                    mask_model_layer = torch.mul(mask_layer, layer)
                    encrypted_mask_model = ts.ckks_tensor(mask_model_layer)
